// ═══════════════════════════════════════════════════════════════
// FILE: lib/screens/dashboards/forensic_dashboard.dart
// ═══════════════════════════════════════════════════════════════
...
class _ForensicDashboardState extends State<ForensicDashboard>
  ...
  void initState() {
    ...
    _pulse = _pulseCtrl.drive(
        Tween(begin: 0.96, end: 1.04).chain(CurveTween(curve: Curves.easeInOut)));
    _loadAll();
  ...
  Widget build(BuildContext context) {
    ...
    return buildDashboardScaffold(
      scaffoldKey: _scaffoldKey, role: 'forensic', email: email, title: 'Forensic Dashboard',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          FadeSlide(delayMs: 0, child: WelcomeBanner(email: email, role: 'forensic',
              subtitle: 'Forensic Expert  ·  Analyze evidence · Upload reports · Monitor risk',
              stats: {'Files': '$total', 'Verified': '$anc', 'Rate': rate}, pulse: _pulse)),
          const SizedBox(height: 16),
          FadeSlide(delayMs: 100, child: StatsGrid(cards: [
            StatCard(C: C, label: 'Evidence Files', value: '$total', sub: 'Assigned for analysis', icon: Icons.biotech_outlined, color: const Color(0xFF7C3AED), loading: _statsLoading, onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
            StatCard(C: C, label: 'Verified', value: '$anc', sub: 'Blockchain confirmed', icon: Icons.verified_outlined, color: const Color(0xFF059669), loading: _statsLoading, onTap: () => _go(const BlockchainViewerScreen())),
            StatCard(C: C, label: 'Pending Cases', value: '${_stats['totalCases'] ?? 0}', sub: 'Awaiting analysis', icon: Icons.pending_outlined, color: const Color(0xFFD97706), loading: _statsLoading, onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
            StatCard(C: C, label: 'Integrity Rate', value: rate, sub: 'Hash verification rate', icon: Icons.security_outlined, color: const Color(0xFF0284C7), loading: _statsLoading, onTap: () => _go(const VerifyEvidenceScreen())),
          ])),
          const SizedBox(height: 16),
          _ResponsiveRow(
            activity: FadeSlide(delayMs: 250, child: GlassCard(padding: const EdgeInsets.all(16), radius: 20, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _h('Recent Activity', Icons.history_rounded, const Color(0xFF7C3AED), onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
              const SizedBox(height: 12),
              if (_actError != null) errorRow(C, _actError!, _loadActivity)
              else if (_actLoading) ...shimmerRows(C, 4)
              else if (_activity.isEmpty) const Padding(padding: EdgeInsets.symmetric(vertical: 16), child: Center(child: Text('No activity yet', style: TextStyle(color: Color(0xFF64748B), fontSize: 13))))
            ...
            actions: FadeSlide(delayMs: 320, child: GlassCard(padding: const EdgeInsets.all(16), radius: 20, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _h('Forensic Actions', Icons.biotech_outlined, const Color(0xFF7C3AED)),
              const SizedBox(height: 12),
              ActionCard(C: C, label: 'View Assigned Evidence', sub: 'Browse case files', icon: Icons.folder_open_outlined, color: const Color(0xFF7C3AED), onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
              const SizedBox(height: 7),
              ActionCard(C: C, label: 'Upload Analysis Report', sub: 'Add forensic findings', icon: Icons.upload_file_outlined, color: const Color(0xFF2563EB), onTap: () => _go(const UploadEvidenceScreen())),
              const SizedBox(height: 7),
              ActionCard(C: C, label: 'Transfer to Prosecutor', sub: 'Forward verified evidence', icon: Icons.swap_horiz_rounded, color: const Color(0xFF059669), onTap: () => _go(CustodyTimelineScreen(evidenceId: null))),
              const SizedBox(height: 7),
              ...
              const SizedBox(height: 7),
              ActionCard(C: C, label: 'View Blockchain Proof', sub: 'On-chain records', icon: Icons.link_rounded, color: const Color(0xFF7C3AED), onTap: () => _go(const BlockchainViewerScreen())),
            ]))),
          ),
...
// ═══════════════════════════════════════════════════════════════
// FILE: lib/screens/dashboards/prosecutor_dashboard.dart
// ═══════════════════════════════════════════════════════════════

// (Copy the same pattern — add risk/simulation imports and nav items)
// For brevity this is documented in the integration guide below.
// Follow the same pattern as forensic_dashboard.dart above.
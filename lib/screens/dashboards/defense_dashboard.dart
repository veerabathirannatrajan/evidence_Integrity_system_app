// defense_dashboard.dart — Uses GlobalKey scaffold, no Scaffold.of() bug, no theme toggle
...
class _DefenseDashboardState extends State<DefenseDashboard> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return buildDashboardScaffold(
      scaffoldKey: _scaffoldKey, role: 'defense', email: email, title: 'Defense Dashboard',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          FadeSlide(delayMs: 0, child: WelcomeBanner(email: email, role: 'defense',
              subtitle: 'Defense Attorney  ·  Examine evidence · Verify authenticity · Challenge tampered records',
              stats: {'Evidence': '$total', 'Verified': '$anc', 'Rate': rate}, pulse: _pulse)),
          const SizedBox(height: 16),
          FadeSlide(delayMs: 100, child: StatsGrid(cards: [
            StatCard(C: C, label: 'Total Evidence',      value: '$total',                       sub: 'Available for review',  icon: Icons.balance_outlined,        color: const Color(0xFF0284C7), loading: _statsLoading, onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
            StatCard(C: C, label: 'Blockchain Verified', value: '$anc',                         sub: 'Immutable records',     icon: Icons.link_rounded,            color: const Color(0xFF7C3AED), loading: _statsLoading, onTap: () => _go(const BlockchainViewerScreen())),
            StatCard(C: C, label: 'Integrity Rate',      value: rate,                           sub: 'Authentication rate',   icon: Icons.verified_user_outlined,  color: const Color(0xFF059669), loading: _statsLoading, onTap: () => _go(const VerifyEvidenceScreen())),
            StatCard(C: C, label: 'Tamper Alerts',       value: '${_stats['tampered'] ?? 0}',   sub: 'Compromised files',     icon: Icons.warning_amber_outlined,  color: const Color(0xFFDC2626), loading: _statsLoading, onTap: () => _go(const VerifyEvidenceScreen())),
          ])),
          const SizedBox(height: 16),
  ...
  Widget _actSec(DC C) => FadeSlide(delayMs: 250, child: GlassCard(padding: const EdgeInsets.all(16), radius: 20,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _h('Recent Activity', Icons.history_rounded, const Color(0xFF0284C7), onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
  ...
  Widget _actionsSec(DC C) => FadeSlide(delayMs: 320, child: GlassCard(padding: const EdgeInsets.all(16), radius: 20,
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _h('Defense Actions', Icons.balance_outlined, const Color(0xFF0284C7)),
        const SizedBox(height: 12),
        ActionCard(C: C, label: 'View Evidence',    sub: 'Browse all case files',         icon: Icons.balance_outlined,        color: const Color(0xFF0284C7), onTap: () => _go(EvidenceListScreen(filterByCaseId: null))),
        const SizedBox(height: 7),
        ActionCard(C: C, label: 'Verify Evidence',  sub: 'Re-upload to check integrity',  icon: Icons.verified_outlined,       color: const Color(0xFF059669), onTap: () => _go(const VerifyEvidenceScreen())),
        const SizedBox(height: 7),
        ActionCard(C: C, label: 'Scan QR Code',     sub: 'Quick evidence lookup',          icon: Icons.qr_code_scanner_rounded, color: const Color(0xFFD97706), onTap: () => _go(const QrScannerScreen())),
        const SizedBox(height: 7),
        ActionCard(C: C, label: 'Blockchain Proof', sub: 'Verify on-chain records',        icon: Icons.link_rounded,            color: const Color(0xFF7C3AED), onTap: () => _go(const BlockchainViewerScreen())),
        const SizedBox(height: 7),
        ActionCard(C: C, label: 'Custody Timeline', sub: 'Full transfer history',          icon: Icons.timeline_outlined,       color: const Color(0xFF2563EB), onTap: () => _go(CustodyTimelineScreen(evidenceId: null))),
        const SizedBox(height: 7),
